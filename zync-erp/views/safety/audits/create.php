<div class="mx-auto max-w-2xl">
    <div class="mb-6 flex items-center justify-between">
        <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-gray-100">Ny audit</h1>
        <a href="/safety/audits" class="text-sm text-gray-500 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">&larr; Tillbaka</a>
    </div>
    <div class="rounded-2xl bg-white dark:bg-gray-800 p-8 shadow-md">
        <form method="POST" action="/safety/audits" class="space-y-5">
            ...
            <div>
                ...
                <select id="template_id" name="template_id" class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
                    ...
                    <?php foreach ($templates as $t): ?>
                        <option value="<?= (int) $t['id'] ?>" <?= ($old['template_id'] ?? '') === (string) $t['id'] ? 'selected' : '' ?>><?= htmlspecialchars($t['name'], ENT_QUOTES, 'UTF-8') ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div>
            ...
            <div>
                ...
                <input id="assigned_to" name="assigned_to" type="number" min="1" required value="<?= htmlspecialchars($old['assigned_to'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
            ...
            <div>
                ...
                <input id="scheduled_date" name="scheduled_date" type="date" required value="<?= htmlspecialchars($old['scheduled_date'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
            ...
            <div class="flex items-center justify-end space-x-3 pt-2">
                <a href="/safety/audits" class="rounded-lg border border-gray-300 dark:border-gray-600 px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">Avbryt</a>
                <button type="submit" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-indigo-700 transition-colors">Skapa audit</button>
            </div>
        </form>
    </div>
</div>
