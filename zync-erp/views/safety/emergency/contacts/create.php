<div class="mx-auto max-w-2xl">
    <div class="mb-6 flex items-center justify-between">
        <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-gray-100">Ny nödkontakt</h1>
    ...
    <div class="rounded-2xl bg-white dark:bg-gray-800 p-8 shadow-md">
        <form method="POST" action="/safety/emergency/contacts" class="space-y-5">
            ...
            <div>
                ...
                <input id="role" name="role" type="text" value="<?= htmlspecialchars($old['role'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
            ...
            <div class="grid grid-cols-2 gap-5">
                <div>
                    ...
                    <input id="phone" name="phone" type="text" required value="<?= htmlspecialchars($old['phone'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
                ...
                <div>
                    ...
                    <input id="phone_alt" name="phone_alt" type="text" value="<?= htmlspecialchars($old['phone_alt'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
                           class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
                </div>
            </div>
            <div>
                <label for="email" class="block text-sm font-medium text-gray-700 dark:text-gray-300">E-post</label>
                <input id="email" name="email" type="email" value="<?= htmlspecialchars($old['email'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
                       class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
            </div>
            <div>
                <label for="organization" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Organisation</label>
                <input id="organization" name="organization" type="text" value="<?= htmlspecialchars($old['organization'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
                       class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
            </div>
            <div>
            ...
            <div>
                ...
                <input id="sort_order" name="sort_order" type="number" min="0" value="<?= htmlspecialchars((string) ($old['sort_order'] ?? '0'), ENT_QUOTES, 'UTF-8') ?>"
                       class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
            </div>
            <div class="flex items-center space-x-2">
                <input id="is_external" name="is_external" type="checkbox" value="1" <?= !empty($old['is_external']) ? 'checked' : '' ?>
            ...
            <div class="flex items-center justify-end space-x-3 pt-2">
                ...
                <button type="submit" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-indigo-700 transition-colors">Skapa kontakt</button>
            </div>
        </form>
    </div>
</div>
