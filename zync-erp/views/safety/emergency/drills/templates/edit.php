<div class="space-y-6 max-w-2xl">
    ...
    <form method="POST" action="/safety/emergency/drills/templates/<?= (int) $template['id'] ?>" class="bg-white dark:bg-gray-800 rounded-2xl shadow-md p-6 space-y-5">
        <?= \App\Core\Csrf::field() ?>

        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Namn <span class="text-red-500">*</span></label>
            <input type="text" name="name" value="<?= htmlspecialchars($template['name'], ENT_QUOTES, 'UTF-8') ?>"
            ...
            <div>
                ...
                <select name="drill_type" class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    <?php foreach (['fire' => 'Brand', 'evacuation' => 'Utrymning', 'chemical' => 'Kemikalieolycka', 'earthquake' => 'Jordbävning', 'lockdown' => 'Låsning', 'medical' => 'Medicinsk nödsituation', 'other' => 'Övrigt'] as $val => $label): ?>
                        <option value="<?= htmlspecialchars($val, ENT_QUOTES, 'UTF-8') ?>" <?= $template['drill_type'] === $val ? 'selected' : '' ?>><?= htmlspecialchars($label, ENT_QUOTES, 'UTF-8') ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Uppskattad tid (min)</label>
                <input type="number" name="duration_estimate" value="<?= htmlspecialchars($template['duration_estimate'] ?? '', ENT_QUOTES, 'UTF-8') ?>" min="0"
        ...
        <div>
            ...
            <textarea name="description" rows="3" class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"><?= htmlspecialchars($template['description'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Checklista</label>
            <textarea name="checklist" rows="5" class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500 font-mono"><?= htmlspecialchars($template['checklist'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nödvändiga resurser</label>
            <textarea name="required_resources" rows="3" class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"><?= htmlspecialchars($template['required_resources'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
        </div>

        <div class="flex items-center gap-2">
            <input type="checkbox" name="is_active" id="is_active" value="1" <?= $template['is_active'] ? 'checked' : '' ?> class="rounded border-gray-300 dark:border-gray-600 text-indigo-600">
