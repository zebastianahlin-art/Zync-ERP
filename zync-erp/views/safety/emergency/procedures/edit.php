<div class="mx-auto max-w-2xl">
    <div class="mb-6 flex items-center justify-between">
        <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-gray-100">Redigera nödprocedur</h1>
        <a href="/safety/emergency/procedures/<?= (int) $procedure['id'] ?>" class="text-sm text-gray-500 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">&larr; Tillbaka</a>
    </div>
    <div class="rounded-2xl bg-white dark:bg-gray-800 p-8 shadow-md">
        <form method="POST" action="/safety/emergency/procedures/<?= (int) $procedure['id'] ?>" class="space-y-5">
            ...
            <div>
                ...
                <input id="title" name="title" type="text" required value="<?= htmlspecialchars($procedure['title'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
            ...
            <div>
                ...
                <input id="responsible" name="responsible" type="text" value="<?= htmlspecialchars($procedure['responsible'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
            ...
            <div>
                ...
                <input id="location" name="location" type="text" value="<?= htmlspecialchars($procedure['location'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
                       class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
            </div>
            <div class="grid grid-cols-2 gap-5">
                <div>
                    <label for="last_reviewed" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Senast granskad</label>
                    <input id="last_reviewed" name="last_reviewed" type="date" value="<?= htmlspecialchars($procedure['last_reviewed'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
                           class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
                </div>
                <div>
                    ...
                    <input id="review_interval" name="review_interval" type="number" min="1" value="<?= htmlspecialchars((string) ($procedure['review_interval'] ?? '365'), ENT_QUOTES, 'UTF-8') ?>"
                           class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
                </div>
            </div>
            <div class="flex items-center space-x-2">
                <input id="is_active" name="is_active" type="checkbox" value="1" <?= !empty($procedure['is_active']) ? 'checked' : '' ?>
                       class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                <label for="is_active" class="text-sm font-medium text-gray-700 dark:text-gray-300">Aktiv</label>
            </div>
            <div class="flex items-center justify-end space-x-3 pt-2">
                <a href="/safety/emergency/procedures/<?= (int) $procedure['id'] ?>" class="rounded-lg border border-gray-300 dark:border-gray-600 px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">Avbryt</a>
