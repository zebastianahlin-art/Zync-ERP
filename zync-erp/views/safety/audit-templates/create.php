<div class="mx-auto max-w-2xl">
    <div class="mb-6 flex items-center justify-between">
        <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-gray-100">Ny auditmall</h1>
        <a href="/safety/audit-templates" class="text-sm text-gray-500 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">&larr; Tillbaka</a>
    </div>
    <div class="rounded-2xl bg-white dark:bg-gray-800 p-8 shadow-md">
        <form method="POST" action="/safety/audit-templates" class="space-y-5">
            ...
            <div>
                ...
                <select id="category" name="category" class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
                    <?php foreach (['workplace'=>'Arbetsplats','fire_safety'=>'Brandsäkerhet','electrical'=>'Elektrisk','chemical'=>'Kemisk','ergonomic'=>'Ergonomisk','environmental'=>'Miljö','general'=>'Allmänt'] as $v=>$l): ?>
                        <option value="<?= $v ?>" <?= ($old['category'] ?? 'general') === $v ? 'selected' : '' ?>><?= $l ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div>
                <label for="version" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Version</label>
                <input id="version" name="version" type="number" min="1" value="<?= htmlspecialchars((string) ($old['version'] ?? '1'), ENT_QUOTES, 'UTF-8') ?>"
            ...
            <div class="flex items-center justify-end space-x-3 pt-2">
                <a href="/safety/audit-templates" class="rounded-lg border border-gray-300 dark:border-gray-600 px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">Avbryt</a>
                <button type="submit" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-indigo-700 transition-colors">Skapa mall</button>
            </div>
        </form>
    </div>
</div>
