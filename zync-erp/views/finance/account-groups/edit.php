<?php $group = $group ?? []; $groups = $groups ?? []; ?>
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Redigera kontoplansgrupp</h1>
        <a href="/finance/account-groups" class="text-sm text-gray-500 hover:text-indigo-600">← Tillbaka</a>
    </div>

    <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
        <form method="POST" action="/finance/account-groups/<?= $group['id'] ?>" class="space-y-4">
            <?= \App\Core\Csrf::field() ?>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" value="<?= htmlspecialchars($group['name'] ?? '') ?>" required class="w-full rounded border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Kod <span class="text-red-500">*</span></label>
                    <input type="text" name="code" value="<?= htmlspecialchars($group['code'] ?? '') ?>" required class="w-full rounded border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-sm font-mono">
                </div>
                <div>
                    ...
                    <select name="parent_id" class="w-full rounded border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-sm">
                        ...
                        <?php if ($g['id'] === $group['id']) continue; ?>
                        <option value="<?= $g['id'] ?>" <?= ($group['parent_id'] ?? '') == $g['id'] ? 'selected' : '' ?>><?= htmlspecialchars($g['code'] . ' — ' . $g['name']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Sorteringsordning</label>
                    <input type="number" name="sort_order" value="<?= (int) ($group['sort_order'] ?? 0) ?>" class="w-full rounded border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-sm">
                </div>
            </div>
