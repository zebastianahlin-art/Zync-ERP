<div class="mx-auto max-w-3xl">
    <div class="mb-6 flex items-center justify-between">
        ...
        <a href="/employees/<?= (int)$employee['id'] ?>" class="text-sm text-gray-500 dark:text-gray-400 hover:text-indigo-600 transition-colors">&larr; Tillbaka</a>
    </div>

    <div class="rounded-2xl bg-white dark:bg-gray-800 p-8 shadow-md">
        <form method="POST" action="/employees/<?= (int)$employee['id'] ?>" class="space-y-5">
            <?= \App\Core\Csrf::field() ?>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input id="end_date" name="end_date" type="date"
                           value="<?= htmlspecialchars($employee['end_date'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
                           class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="employment_type" name="employment_type"
                        ...
                        <option value="full_time" <?= ($employee['employment_type'] ?? '') === 'full_time' ? 'selected' : '' ?>>Heltid</option>
                        <option value="part_time" <?= ($employee['employment_type'] ?? '') === 'part_time' ? 'selected' : '' ?>>Deltid</option>
                        <option value="consultant" <?= ($employee['employment_type'] ?? '') === 'consultant' ? 'selected' : '' ?>>Konsult</option>
                        <option value="intern" <?= ($employee['employment_type'] ?? '') === 'intern' ? 'selected' : '' ?>>Praktikant</option>
                    </select>
                </div>
                <div>
                    ...
                    <select id="status" name="status"
                            class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
                        <option value="active" <?= ($employee['status'] ?? 'active') === 'active' ? 'selected' : '' ?>>Aktiv</option>
                        <option value="on_leave" <?= ($employee['status'] ?? '') === 'on_leave' ? 'selected' : '' ?>>Tj&#228;nstledig</option>
                        <option value="terminated" <?= ($employee['status'] ?? '') === 'terminated' ? 'selected' : '' ?>>Avslutad</option>
                    </select>
                </div>
            </div>
            ...
            <div>
                ...
                <input id="salary" name="salary" type="number" step="0.01" min="0"
                       value="<?= htmlspecialchars((string)($employee['salary'] ?? ''), ENT_QUOTES, 'UTF-8') ?>"
                       class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="emergency_contact_name" name="emergency_contact_name" type="text"
                           value="<?= htmlspecialchars($employee['emergency_contact_name'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
                           class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
                </div>
                <div>
                    ...
                    <input id="emergency_contact_phone" name="emergency_contact_phone" type="text"
                           value="<?= htmlspecialchars($employee['emergency_contact_phone'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
                           class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea id="notes" name="notes" rows="3"
                          class="mt-1 block w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"><?= htmlspecialchars($employee['notes'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
            </div>

            <div class="flex items-center justify-end space-x-3 pt-2">
                <a href="/employees/<?= (int)$employee['id'] ?>" class="rounded-lg border border-gray-300 dark:border-gray-600 px-4 py-2 text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">Avbryt</a>
