<div class="space-y-6">
    <div class="flex items-center gap-3">
        <a href="/maintenance/work-orders/<?= (int)$workOrder['id'] ?>" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
        </a>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Redigera arbetsorder</h1>
    </div>
    ...
    <form method="POST" action="/maintenance/work-orders/<?= (int)$workOrder['id'] ?>" class="space-y-6">
        <?= \App\Core\Csrf::field() ?>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-6 space-y-5">
            ...
            <div>
                ...
                <input type="text" id="title" name="title" required
                       value="<?= htmlspecialchars($_POST['title'] ?? $workOrder['title'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <select id="work_type" name="work_type" required class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">
                        ...
                        $selWorkType = $_POST['work_type'] ?? $workOrder['work_type'] ?? 'corrective';
                        foreach ($workTypes as $val => $label):
                        ?>
                        <option value="<?= htmlspecialchars($val, ENT_QUOTES, 'UTF-8') ?>" <?= $selWorkType === $val ? 'selected' : '' ?>>
                ...
                <div>
                    ...
                    <select id="priority" name="priority" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">
                        ...
                        $selPriority = $_POST['priority'] ?? $workOrder['priority'] ?? 'normal';
                        foreach ($priorities as $val => $label):
                        ?>
                        <option value="<?= htmlspecialchars($val, ENT_QUOTES, 'UTF-8') ?>" <?= $selPriority === $val ? 'selected' : '' ?>>
                            <?= htmlspecialchars($label, ENT_QUOTES, 'UTF-8') ?>
                        </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div>
                    ...
                    <select id="machine_id" name="machine_id" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">
                        ...
                        <option value="<?= (int)$m['id'] ?>" <?= (($_POST['machine_id'] ?? $workOrder['machine_id'] ?? '') == $m['id']) ? 'selected' : '' ?>>
                ...
                <div>
                    ...
                    <select id="equipment_id" name="equipment_id" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">
                        ...
                        <option value="<?= (int)$e['id'] ?>" <?= (($_POST['equipment_id'] ?? $workOrder['equipment_id'] ?? '') == $e['id']) ? 'selected' : '' ?>>
                ...
                <div>
                    ...
                    <select id="fault_report_id" name="fault_report_id" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">
                        ...
                        <option value="<?= (int)$fr['id'] ?>" <?= (($_POST['fault_report_id'] ?? $workOrder['fault_report_id'] ?? '') == $fr['id']) ? 'selected' : '' ?>>
                ...
                <div>
                    ...
                    <input type="text" id="location" name="location"
                           value="<?= htmlspecialchars($_POST['location'] ?? $workOrder['location'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
                           class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">
                </div>
                ...
                <div>
                    ...
                    <select id="department_id" name="department_id" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">
                        ...
                        <option value="<?= (int)$d['id'] ?>" <?= (($_POST['department_id'] ?? $workOrder['department_id'] ?? '') == $d['id']) ? 'selected' : '' ?>>
                ...
                <div>
                    ...
                    <select id="cost_center_id" name="cost_center_id" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">
                        ...
                        <option value="<?= (int)$cc['id'] ?>" <?= (($_POST['cost_center_id'] ?? $workOrder['cost_center_id'] ?? '') == $cc['id']) ? 'selected' : '' ?>>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="datetime-local" id="planned_start" name="planned_start"
                           value="<?= htmlspecialchars($_POST['planned_start'] ?? (isset($workOrder['planned_start']) ? date('Y-m-d\TH:i', strtotime($workOrder['planned_start'])) : ''), ENT_QUOTES, 'UTF-8') ?>"
                           class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">
                </div>
                ...
                <div>
                    ...
                    <input type="datetime-local" id="planned_end" name="planned_end"
                           value="<?= htmlspecialchars($_POST['planned_end'] ?? (isset($workOrder['planned_end']) ? date('Y-m-d\TH:i', strtotime($workOrder['planned_end'])) : ''), ENT_QUOTES, 'UTF-8') ?>"
                           class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="estimated_hours" name="estimated_hours" step="0.5" min="0"
                           value="<?= htmlspecialchars($_POST['estimated_hours'] ?? $workOrder['estimated_hours'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
                           class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">
                </div>

                <!-- Tilldelad till -->
                <div>
                    <label for="assigned_to" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tilldelad till</label>
                    <select id="assigned_to" name="assigned_to" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">
                        ...
                        <option value="<?= (int)$u['id'] ?>" <?= (($_POST['assigned_to'] ?? $workOrder['assigned_to'] ?? '') == $u['id']) ? 'selected' : '' ?>>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-6 space-y-5">
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="5"
                          class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm"><?= htmlspecialchars($_POST['description'] ?? $workOrder['description'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
            </div>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="/maintenance/work-orders/<?= (int)$workOrder['id'] ?>" class="px-5 py-2 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition">
                Avbryt
            </a>
        </div>
    </form>
