            ...
            <div class="flex items-start justify-between gap-4">
                <div>
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-xs text-gray-500 dark:text-gray-400 font-mono"><?= htmlspecialchars($wo['wo_number'], ENT_QUOTES, 'UTF-8') ?></span>
                    </div>
                    <div class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        Utfört av: <strong><?= htmlspecialchars($wo['assigned_to_name'] ?? '—', ENT_QUOTES, 'UTF-8') ?></strong>
                        · Utrustning: <strong><?= htmlspecialchars($wo['equipment_name'] ?? '—', ENT_QUOTES, 'UTF-8') ?></strong>
                    </div>
                    <div class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        Faktiska timmar: <strong><?= htmlspecialchars((string)($wo['actual_hours'] ?? '—'), ENT_QUOTES, 'UTF-8') ?></strong>
                    </div>
                </div>
                    ...
                    <div class="relative inline-block" x-data="{ open: false }">
                        <form method="POST" action="/maintenance/work-orders/<?= $wo['id'] ?>/reject">
                            ...
                            <div x-show="open" @click.outside="open=false" class="absolute right-0 mt-2 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 z-10 min-w-64">
                                <textarea name="notes" rows="2" required placeholder="Anledning..." class="w-full rounded border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white text-sm mb-2"></textarea>
