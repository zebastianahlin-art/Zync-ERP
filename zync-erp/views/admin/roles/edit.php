<div class="max-w-md space-y-6">
    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Redigera roll</h1>

    <form method="POST" action="/admin/roles/<?= $role['id'] ?>" class="bg-white dark:bg-gray-800 rounded-xl shadow p-6 space-y-4">
        <?= \App\Core\Csrf::field() ?>

        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Namn <span class="text-red-500">*</span></label>
            <input type="text" name="name" value="<?= htmlspecialchars($role['name'], ENT_QUOTES, 'UTF-8') ?>"
        ...
        <div>
            ...
            <input type="text" name="slug" value="<?= htmlspecialchars($role['slug'], ENT_QUOTES, 'UTF-8') ?>"
        ...
        <div>
            ...
            <input type="number" name="level" value="<?= (int) $role['level'] ?>" min="1" max="10"
                class="w-full border border-gray-300 dark:border-gray-600 dark:bg-gray-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
