<div class="space-y-6 max-w-2xl">
    <div>
        <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-gray-100">Redigera reseräkning</h1>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5"><?= htmlspecialchars($report['report_number'], ENT_QUOTES, 'UTF-8') ?></p>
    </div>
    ...
    <form method="POST" action="/hr/expenses/<?= (int) $report['id'] ?>" class="bg-white dark:bg-gray-800 rounded-2xl shadow-md p-6 space-y-5">
        <?= \App\Core\Csrf::field() ?>
        ...
        <div>
            ...
            <select name="employee_id" required class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                ...
                <?php foreach ($employees as $emp): ?>
                    <option value="<?= (int) $emp['id'] ?>" <?= $report['employee_id'] == $emp['id'] ? 'selected' : '' ?>><?= htmlspecialchars($emp['full_name'], ENT_QUOTES, 'UTF-8') ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <div>
            ...
            <input type="text" name="title" value="<?= htmlspecialchars($report['title'], ENT_QUOTES, 'UTF-8') ?>"
        ...
        <div>
            ...
            <textarea name="description" rows="3" class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"><?= htmlspecialchars($report['description'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
        </div>

        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Resa start</label>
                <input type="date" name="trip_start" value="<?= htmlspecialchars($report['trip_start'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
            ...
            <div>
                ...
                <input type="date" name="trip_end" value="<?= htmlspecialchars($report['trip_end'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
                       class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500">
            </div>
        </div>
        ...
        <div>
            ...
            <input type="text" name="destination" value="<?= htmlspecialchars($report['destination'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
                   class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500">
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Syfte</label>
            <input type="text" name="purpose" value="<?= htmlspecialchars($report['purpose'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
                   class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500">
        </div>
        ...
        <div>
            ...
            <select name="currency" class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                <option value="SEK" <?= $report['currency'] === 'SEK' ? 'selected' : '' ?>>SEK</option>
                <option value="EUR" <?= $report['currency'] === 'EUR' ? 'selected' : '' ?>>EUR</option>
        ...
        <div>
            ...
            <textarea name="notes" rows="2" class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"><?= htmlspecialchars($report['notes'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
        </div>

        <div class="flex justify-end gap-3 pt-2">
            <a href="/hr/expenses/<?= (int) $report['id'] ?>" class="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:underline">Avbryt</a>
