<div class="max-w-2xl space-y-6">
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Redigera order</h1>
    </div>
    <form method="post" action="/sales/orders/<?= $item['id'] ?>" class="space-y-5 rounded-xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" name="order_date" value="<?= htmlspecialchars($item['order_date'] ?? '') ?>" class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Leveransdatum</label>
                <input type="date" name="delivery_date" value="<?= htmlspecialchars($item['delivery_date'] ?? '') ?>" class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none">
            </div>
        </div>
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <select name="status" class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none">
                    <option value="confirmed" <?= $item['status'] === 'confirmed' ? 'selected' : '' ?>>Bekräftad</option>
                    <option value="in_production" <?= $item['status'] === 'in_production' ? 'selected' : '' ?>>I produktion</option>
                    <option value="shipped" <?= $item['status'] === 'shipped' ? 'selected' : '' ?>>Skickad</option>
        ...
        <div class="flex gap-3 pt-2">
            ...
            <a href="/sales/orders" class="rounded-lg border border-gray-300 dark:border-gray-600 px-5 py-2 text-sm font-semibold text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700">Avbryt</a>
        </div>
    </form>
</div>
