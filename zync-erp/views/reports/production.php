...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Produktionsrapport</h1>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-4 text-center"><div class="text-3xl font-bold text-indigo-600 dark:text-indigo-400"><?= (int)($s['total'] ?? 0) ?></div><div class="text-sm text-gray-500 mt-1">Totalt ordrar</div></div>
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-4 text-center"><div class="text-3xl font-bold text-orange-500"><?= (int)($s['active_orders'] ?? 0) ?></div><div class="text-sm text-gray-500 mt-1">Aktiva</div></div>
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-4 text-center"><div class="text-3xl font-bold text-green-500"><?= (int)($s['completed_orders'] ?? 0) ?></div><div class="text-sm text-gray-500 mt-1">Avslutade</div></div>
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-4 text-center"><div class="text-3xl font-bold text-blue-500"><?= (int)($s['products'] ?? 0) ?></div><div class="text-sm text-gray-500 mt-1">Produkter</div></div>
    </div>
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow overflow-x-auto">
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                ...
                <?php endforeach; ?>
                <?php if (empty($data)): ?><tr><td colspan="2" class="px-4 py-6 text-center text-gray-400">Inga produktionsordrar för vald period.</td></tr><?php endif; ?>
            </tbody>
        </table>
    </div>
</div>
